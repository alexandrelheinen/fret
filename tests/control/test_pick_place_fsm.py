...
def test_fsm_faults_on_drop_during_transfer() -> None:
    wp = waypoints_from_scenario()
    fsm = PickPlaceFSM(wp, joint_tol_rad=0.05, lift_height_m=0.12)
    fsm.start()
    fsm._enter(PickPlaceState.MOVE_PLACE)  # noqa: SLF001
    obs = PickPlaceObservation(
        q=wp.pick_hover,
        object_pos=np.array([0.08, 0.0, 0.02]),
        ee_pos=np.array([0.08, 0.0, 0.17]),
    )
    fsm.tick(obs, 0.05)
...
def test_omx_pick_place_physics_moves_box_green_to_red() -> None:
    ...
    state, box = run_pick_place(duration_s=20.0)
    ...
    assert float(box[2]) < 0.14, "box should rest near the place pedestal"
