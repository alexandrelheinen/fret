#!/usr/bin/env python3
"""Import TurtleBot3 Burger meshes for Dubins race vehicle visuals.

Vendors STL meshes from the ROBOTIS MuJoCo Menagerie (Apache-2.0) into
``src/fret/mjcf/assets/dubins/turtlebot3/``.

Source: https://github.com/ROBOTIS-GIT/robotis_mujoco_menagerie

Example::

    python3 scripts/import_dubins_assets.py
    python3 scripts/import_dubins_assets.py --src /path/to/robotis_mujoco_menagerie
"""
...
_DEFAULT_OUT = _REPO_ROOT / "src/fret/mjcf/assets/dubins/turtlebot3"

# Meshes used for kinematic Dubins race agents (visual only in MJCF).
_MESHES: tuple[str, ...] = (
    "burger_base.stl",
    "left_tire.stl",
    "right_tire.stl",
...
def import_assets(src_root: Path, out_dir: Path) -> None:
    ...
    license_note = out_dir / "LICENSE"
    if not license_note.exists():
        license_note.write_text(
            "TurtleBot3 meshes from ROBOTIS MuJoCo Menagerie.\n"
